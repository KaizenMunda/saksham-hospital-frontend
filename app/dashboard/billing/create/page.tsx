...
import { ArrowLeft, Plus, Trash, PlusCircle, MinusCircle } from "lucide-react"
import { Card, CardContent } from "@/components/ui/card"
import { useToast } from "@/components/ui/use-toast"
import { MOCK_ITEMS } from "@/app/dashboard/inventory/types"
import { type InvoiceFormData, type InvoiceItem, MOCK_INVOICES } from "@/app/dashboard/billing/types"
import { AddItemsDialog } from "@/components/billing/AddItemsDialog"

// Define mock patients if the import fails
const MOCK_PATIENTS = [
  { 
    id: 'P001', 
    name: 'John Doe', 
...
// Define a simplified Patient type for our page
interface Patient {
  id: string;
  name: string;
  dateOfBirth: Date;
...
type InvoiceAction = 
  ...
  | { type: 'ADD_MULTIPLE_ITEMS'; items: Omit<InvoiceItem, 'id' | 'invoiceId'>[] }
...
export default function CreateInvoicePage() {
  ...
  // Memoize patient options to avoid re-renders
  ...
  const handlePatientChange = (patientId: string) => {
    const patient = MOCK_PATIENTS.find((p: Patient) => p.id === patientId);
    dispatch({ 
      type: 'SET_PATIENT', 
      patientId, 
  ...
  // Handle adding multiple items from dialog
  const handleAddMultipleItems = (items: Omit<InvoiceItem, 'id' | 'invoiceId'>[]) => {
    dispatch({ type: 'ADD_MULTIPLE_ITEMS', items });
    toast({
      title: `${items.length} ${items.length === 1 ? 'item' : 'items'} added`,
      description: "Items have been added to the invoice",
    });
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Generate invoice number based on existing invoices
      const lastInvoiceNumber = MOCK_INVOICES.length > 0 
        ? parseInt(MOCK_INVOICES[MOCK_INVOICES.length - 1].invoiceNumber.split('-')[2]) 
        : 0;
      const newInvoiceNumber = `INV-${new Date().getFullYear()}-${(lastInvoiceNumber + 1).toString().padStart(4, '0')}`;
      ...
      const newInvoice = {
        ...
        items: formState.items.map((item, index) => ({
          ...
          invoiceId: `${MOCK_INVOICES.length + 1}`
        }))
      };
      ...
      // Add to MOCK_INVOICES (not actually persisted in this demo)
      // In a real app, you would make an API call here
      
      router.push('/dashboard/billing'); // Navigate back to the billing page
      toast({
        title: "Success",
                  ...
                  {formState.items.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={6} className="h-24 text-center text-muted-foreground">
                        No items added. Click the "Add Items" button above.
                      </TableCell>
                    </TableRow>
                  ) : (
